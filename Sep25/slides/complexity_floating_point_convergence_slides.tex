...
loop pattern & cost \\
\midrule
fixed number of rounds & $O(1)$ \\
\texttt{i += c} & $O(n)$ \\
\texttt{i *= 2} & $O(\log n)$ \\
two nested loops to $n$ & $O(n^2)$ \\
loop, then another loop & $O(n)$ \\
\bottomrule
\end{tabular}

\vspace{0.3cm}
\begin{block}{The doubling test}
Double $n$ and watch the flop count: $\times2 \Rightarrow n$, \ $\times4 \Rightarrow n^2$, \ $\times8 \Rightarrow n^3$, \ $+1 \Rightarrow \log n$.
\end{block}
\end{column}
...
\begin{tikzpicture}
\begin{axis}[
  width=4.4cm, height=4.6cm, axis lines=left, xmin=1, xmax=20, ymin=0, ymax=100,
...
\end{axis}
\end{tikzpicture}\\[2pt]
\scriptsize Big-$O$ also describes \textbf{errors}: later we will write\\ error $= O(1/n)$ or $O(0.67^n)$.
\end{column}
\end{columns}
\end{frame}
